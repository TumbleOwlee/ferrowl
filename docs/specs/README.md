...
Most requirements are pinned by a test citing the ID. This list records the deliberate exceptions. Three kinds qualify:
...
Anything not listed below must carry a citing test. A listed requirement that gains observable behavior: remove from the list, add the test.
