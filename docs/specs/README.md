...
2. **IDs stable, append-only.** `<PREFIX>-R-nnn` for a requirement, `<PREFIX>-E-nnn` for an `edge-cases.md` entry; prefix per area from `AGENTS.md`'s routing table (`NF` has no `-E` series). The two series are numbered independently per area; never renumber, never reuse a retired ID. Sole exception: one ID defining two different entries, which no other repair can make unique — the definition with fewer citations in the sources takes the next free ID in its series, keeping its wording and its position in the file, and every citation of it is updated in the same change; the ID that was duplicated keeps the other definition, so nothing is retired, freed or reused. An `-E` entry derived from a requirement cites that `-R` ID in its text and still carries its own `-E` ID. Tests cite `-E` IDs exactly as `-R` IDs (rule 8).
...
| `CS-R-053`, `CS-E-008` (resolved-path clause only) — the skip warning names the resolved device path, not the raw string | the diagnostic is `eprintln!`d in `ferrowl/src/main.rs`'s `build_tabs`, reached only after the TUI's raw-mode entry point (`UI-R-001`) is next in the same code path, so asserting the literal text needs either an in-process stderr capture this repo has no harness for or a subprocess that risks leaving the caller's terminal in raw mode; the resolved value itself is pinned by `CliArgs::module_specs`'s own tests (`ut_session_device_resolves_against_session_dir` et al.), and skip-not-abort by `ut_missing_device_is_skipped_not_fatal` |
| `CS-E-009` (end-to-end clause) — `:write` with no path actually lands at `session.toml` in the real process cwd | `:write` is reachable only through the interactive `ModuleView`/`App` loop behind `UI-R-001`'s raw-mode entry point, so a subprocess drive of it risks leaving the caller's terminal in raw mode the same way `CS-R-053`'s would; the default target's base-directory computation is pinned in-process by `ut_write_default_target_base_is_process_cwd` (`ferrowl/src/app/commands.rs`), and the relativization it feeds into by `ut_write_relativizes_device_under_target_dir` |
...
| `NF-R-067`, `NF-R-068` — non-blocking command dispatch and stop-aborts-attempt | `UI-R-314`/`UI-R-315`/`UI-R-350`, `MB-R-220`/`MB-R-221`/`MB-R-255`, `OC-R-175`/`OC-R-176`/`OC-R-177` |
