...
| A retransmitted `WriteSingleRegister`/`WriteSingleCoil` request arrives while the monitor is still waiting for that request's response | decodes as the response to itself, since the two are byte-identical on the wire — produces a false `Ok` record and a phantom observed-table write; `WriteMultiple*` and the read operations are unaffected, since their request/response encodings differ |
| Two configured module instances share the same Rtu/Ascii path | MB-R-150's cross-check catches this before the OS-level open and reports a distinct conflict status, recovering automatically once one instance stops or reconfigures off the path |
| The serial path is held open by something the session doesn't know about (a process outside ferrowl, or a bridge-mode downstream/upstream) | MB-R-150 only sees module instances configured in the same session; an external holder still surfaces as an ordinary OS-level open failure/retry, same as before |
...
| The Modbus TCP setup dialog's client-role Skip-Verify toggled On after Root Store/CA list state was entered | resolved verification excludes both the Root Store selection and the CA list entirely, becoming `CertVerification::Skip` (MB-R-135) |
...
| A `cert_file`/`key_file`/`ca_files`/`extra_ca_files`/client-identity path that is malformed PEM or unreadable | server or client start fails with a TLS configuration error, the same tier as MB-R-107/MB-R-108 |
| A `ServerTlsPolicy::Mutual` client certificate signed by any one of several configured `ca_files` | accepted — verification is "any one matches", not "all must match"; `ca_files` is a trust-anchor set, not an ordered chain (MB-R-108) |
| `CertVerification::CaFiles` constructed with an empty `ca_files` list | rejected by the policy's construction-time check, never at handshake time (MB-R-108/MB-R-109) |
| `ServerTlsPolicy::Mutual` with `CertVerification::Skip` and a connection presenting no client certificate at all | handshake still fails — `Skip` skips the CA/identity check on a *presented* cert, it does not make presenting one optional (MB-R-108) |
| The Modbus TCP setup dialog's server-role Skip Verify toggled On while mTLS is selected | the shared CA list widget is hidden and the resolved verification becomes `CertVerification::Skip` regardless of entries already present; list contents preserved, restored when the toggle goes back Off (MB-R-136) |
| The Modbus TCP setup dialog's client-role Self Signed toggled On while mTLS is selected | the Client Cert/Key inputs are hidden and the resolved identity becomes `CertSource::SelfSigned` regardless of text already present; text preserved, restored when the toggle goes back Off (MB-R-139) |
...
| A client-role `CertVerification::CaFiles` with an empty `ca_files` | Refused at construction and at dialog submit (MB-R-109/MB-R-156): the resulting trust store would reject every server certificate, so "trust nothing" is a misconfiguration rather than an honored setting. `RootStore` needs no such rule — it always trusts the platform roots, so its `extra_ca_files` may legitimately be empty |
| `CertVerification::RootStore` on a server's client-certificate verification | Rejected at construction; the variant is client-only and no dialog offers it (MB-R-108). The platform root store is a serverAuth trust list, and many publicly issued end-entity certificates also carry the clientAuth EKU, so honoring it here would accept any client holding a publicly trusted certificate for any domain it controls — a mutual-TLS bypass, not an authentication mode. Private-CA trust anchors are the only sanctioned client-certificate source |
| `CertSource::Ephemeral` as a client's mTLS identity | Rejected at construction: `Ephemeral` means "nothing was configured, fall back and log", which is a server-side listener behavior. A client with no identity is `ClientTlsPolicy::Tls`, not `Mutual` |
