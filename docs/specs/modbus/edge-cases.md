...
| A monitor's serial path is already held open by an RTU/ASCII client or server | the monitor opens through the same entry point a competing client/server open would, so it fails/retries the same way — a monitor cannot share a serial path with another opener; an optional start-time cross-check against other configured devices on the same path remains a follow-up, not implemented here |
