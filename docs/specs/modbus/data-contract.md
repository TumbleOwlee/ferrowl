...
- Each 16-bit word is always transmitted high byte first — Modbus wire format, not configurable.
...
Composes with byte order as a separate axis: on decode words are reordered **first**, then §2.2's byte-order rule; on encode byte-order first, reorder last. Exact inverses. Default `Normal` = byte-order rule alone.
...
- Two characters per register; block is exactly `2 × length` bytes.
- `Alignment` `Left` or `Right` governs **padding and truncation on encode**: `Left` writes from the first byte, zero-pads right, over-long input keeps the **first** `2 × length` bytes; `Right` zero-pads left, keeps the **last** `2 × length` bytes.
...
- **Shift derived** from the mask as its trailing-zero count; never configured independently.
...
- **write mask**: one 16-bit word per register of the format's width, carrying the bits it owns, laid out in the same byte order the value is encoded with. Full-width integer, float, or ASCII yields all-ones words.
...
- **Fixed(u16)** — 0–65535. Occupies `[address, address + format width)` in its (slave, table) space.
...
- never occupy store memory, never read from or written to the wire;
...
- Ranges **half-open**: `[start, end)`, `length = end - start`.
...
- **Without** `read_ranges` for that code: each register its own request. Contiguous registers **not** merged.
- **With** `read_ranges`: every register inside one configured range is read by a single request bridging their gaps, trimmed to the first and last register's extent — leading/trailing empty space not read. Registers outside every range get their own requests.
