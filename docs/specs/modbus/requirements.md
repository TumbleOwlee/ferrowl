...
**MB-R-136** — The Modbus TCP setup dialog's client-CA input, shown for the server role whenever mTLS is selected, shall allow zero or more CA file paths to be added, edited, or removed individually, mirroring the add/remove/edit list pattern already used for a register's predefined named values. Confirming an add-entry attempt shall be rejected — leaving the entry sub-dialog open with an inline error instead of appending anything — unless the typed path is non-empty, exists on disk, is not a directory, and has one of the accepted certificate-file extensions (`pem`/`crt`/`key`, case-insensitive). Selecting mTLS with a non-empty list and Skip Verify Off resolves to `ServerTlsPolicy::MutualTls` with `ClientCertVerification::Verify` holding exactly those files; an empty list at that point shall be a validation error (mirroring MB-R-108), not a silent fallback to `Tls`. The dialog shall additionally offer a Skip Verify toggle for the server role, shown whenever mTLS is selected: when On, the Client CA list input shall be hidden and excluded from the resolved config regardless of entries already present, resolving to `ClientCertVerification::SkipVerify` instead — mirroring MB-R-135's hidden-field-exclusion pattern (list contents preserved, restored when the toggle goes back Off).

**MB-R-138** — Under `ClientTlsPolicy::MutualTls` with `ClientCertSource::SelfSigned`, a client shall present an ephemeral self-signed certificate/key pair as its mTLS identity, generated and cached per the rule in MB-R-106 (generated once per module instance, reused across reconnects/restarts/config edits, regenerated only on a transition into self-signed), never written to disk.
...
**MB-R-105** — A Modbus TCP/`RtuOverTcp`/`AsciiOverTcp` endpoint's TLS configuration shall be represented as one of two role-specific policy enums instead of a single struct carrying both roles' fields: `enum ServerTlsPolicy { NoTls, Tls { server_cert: ServerCertSource }, MutualTls { server_cert: ServerCertSource, client_verification: ClientCertVerification } }` and `enum ClientTlsPolicy { NoTls, Tls { client_verification: ClientVerification }, MutualTls { client_verification: ClientVerification, client_identity: ClientCertSource } }`, where `enum ClientCertVerification { Verify { ca_files: Vec<String> }, SkipVerify }` (server role: verifying an incoming client cert under mTLS; `ca_files` non-empty, construction-time checked) and `enum ClientCertSource { SelfSigned, Explicit { client_cert_file: String, client_key_file: String } }` (client role: the identity a client presents under mTLS). A server-role endpoint's TLS field is a `ServerTlsPolicy`; a client-role endpoint's is a `ClientTlsPolicy`. Neither carries the other role's fields. `NoTls` replaces the endpoint's former `Option<...>`-is-`None` state. `MutualTls`'s extra payload is required (not `Option`) exactly when that variant is chosen: `ClientCertVerification::Verify`'s `ca_files` must be non-empty, and `ClientCertSource::Explicit`'s `client_cert_file`/`client_key_file` must both be present — both enforced at the same `resolve()`-time construction check `ServerCertSource`/`ClientVerification` already use. `OcppSecurityConfig`/`CsmsTlsConfig`/`CsTlsConfig` adopt the same `ServerTlsPolicy`/`ClientTlsPolicy` split for their respective roles (OC-R-039/OC-R-096).

**MB-R-106** — With `tls` set, a server shall resolve its presented certificate as: an ephemeral self-signed certificate when `self_signed` is set (`cert_file`/`key_file` are then structurally unreachable, regardless of any value present); otherwise the PEM files at `cert_file`/`key_file` when both are set; otherwise an ephemeral self-signed certificate with the fallback logged, when none of `cert_file`, `key_file`, `self_signed` is set. A self-signed pair (from either branch above) shall be generated once and cached for the life of the module instance; every subsequent bind attempt, reconnect-driven rebind, and `:restart`/`:reload` shall reuse the cached pair, including across a configuration edit that leaves the resolved source as self-signed. The pair shall be regenerated only when the resolved source changes *to* self-signed from something else — explicit files removed, `self_signed` toggled Off→On, or `tls` transitioning from unset to set while resolving to self-signed. A module instance being torn down and freshly constructed (not merely `:restart`/`:reload` of the same instance) discards the cache, since the pair is never persisted to disk. This resolve-time fallback governs a config file loaded outside the setup dialog; the dialog itself additionally refuses to submit in this same "none set" case (MB-R-107).

**MB-R-107** — `cert_file` or `key_file` set alone (not both), while `self_signed` is not set, shall fail configuration resolution with a TLS configuration error, rather than silently falling through to a self-signed certificate. Once `self_signed` is set, `cert_file`/`key_file` are structurally unreachable (MB-R-106) and this rule does not apply. The setup dialog shows an error border on `cert_file`/`key_file`/`client_cert_file`/`client_key_file` while blank and shown, and — while `self_signed` is not set — refuses to close the dialog on submit if either of a role's pair is blank, distinct from MB-R-106's own resolve-time fallback for a config file loaded outside the dialog.

**MB-R-108** — With a server endpoint's `ServerTlsPolicy::MutualTls`, verification of the presented client certificate is governed by `client_verification`. Under `ClientCertVerification::Verify`, a connection presenting no client certificate, or one not signed by any of the configured `ca_files`, shall fail the TLS handshake and never reach the request handler — a certificate signed by any one configured CA is sufficient, not all; `Verify` shall carry at least one CA file, construction fails otherwise. Under `ClientCertVerification::SkipVerify`, a connection presenting no client certificate shall still fail the handshake, but a presented certificate shall be accepted without CA validation — the handshake's signature check is still performed, only the chain/identity check is skipped. `ServerTlsPolicy::Tls` shall never request a client certificate — `MutualTls` is the sole trigger, in either verification mode.
