...
- **stderr** carries setup/fatal diagnostics (`Error:`/`error:`, the TUI's module-skip warnings) and the per-module teardown lines `Stopped '<name>'` / `Error: failed to stop '<name>': <detail>` / `Error: timed out stopping '<name>'` (CL-R-056, CL-R-057, CL-R-058, CL-R-059), so stdout stays parseable (CL-R-042).
