...
- **CL-E-028** — **Headless has no per-module error isolation.** Any one module's startup failure fails the whole `run` with exit 1 (`## 5. Session / device load failures in headless`); no "start the good ones, report the bad" mode.
