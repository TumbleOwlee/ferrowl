...
- **CL-E-027** — **`--exit-on-error` only catches logged Error-level lines.** A level match, not a structured result channel (`## Exit codes`). Errors never reaching the log, or logged below Error, are invisible.
