...
Any other Lua value (`table`, `function`, `userdata`, `thread`) where a scalar is expected raises a conversion error. Sole structured argument: the optional OCPP action **override table** (``## `C_OCPP` — OCPP state access and action dispatch``), whose entries are flat scalars.
...
## Modules available per script context
...
`C_Statics` (``## `C_Log` and `C_Statics` ``) is library surface but **not** registered by any ferrowl sim; no ferrowl script can call it.

---

## `C_Register` — Modbus register access
...
### Server shape (CSMS spanning many stations)
...
Returned by `Connector(...)` / `ChargingStation(...)`. Not a global. Exposes exactly ``### Shared surface (all shapes, and every `Accessor`)``'s surface scoped to one connector or station.
...
### `ModuleHandle` (return of `C_Module:Get`)
...
## Type coercion on `C_Register:Set`
...
### `C_Statics` — read-only constants (library surface, not wired)
