...
**OC-R-034** — A CS TLS configuration shall trust the webpki root store, plus the certificates in a configured `ca_file` when one is set.

**OC-R-035** — A CS shall present a client certificate for mutual TLS when, and only when, both a client certificate file and its matching key file are configured.

**OC-R-036** — A CS with `insecure_skip_verify` set shall accept any server certificate without authenticating it, and shall ignore `ca_file` entirely. The handshake's signature verification shall still be performed; only the certificate-chain/identity check is skipped.

**OC-R-037** — A CSMS TLS server certificate shall come either from PEM files on disk or from an ephemeral self-signed certificate. The self-signed pair shall be generated once and cached for the life of the module instance, reused across every bind attempt, reconnect-driven rebind, and `:restart`/`:reload`, and across a configuration edit that leaves `self_signed` On — regenerated only when `self_signed` transitions Off→On, or the endpoint's TLS mode transitions from `NoTls` to `Tls`/`MutualTls` while `self_signed` is already On. A module instance being torn down and freshly constructed discards the cache. Never written to disk.
...
**OC-R-039** — With a CSMS's `ServerTlsPolicy::MutualTls`, verification of the presented client certificate is governed by `client_verification`. Under `ClientCertVerification::Verify`, a client presenting no certificate, or one not signed by any of the configured `ca_files`, shall be rejected — a certificate signed by any one configured CA is sufficient, not all; `Verify` shall carry at least one CA file, construction fails otherwise. Under `ClientCertVerification::SkipVerify`, a client presenting no certificate shall still be rejected, but a presented certificate shall be accepted without CA validation — signature check still performed, only chain/identity check skipped.

**OC-R-040** — `ServerTlsPolicy::MutualTls` combined with a self-signed CSMS certificate (`server_cert: ServerCertSource::SelfSigned`) shall be permitted regardless of which `client_verification` mode is configured — `ClientCertVerification::Verify` (with at least one CA file, construction-checked) or `ClientCertVerification::SkipVerify` — since the server's own self-signed identity and its client-verification mode are independent. `Verify` with zero CA files remains unrepresentable (construction fails, OC-R-039), regardless of certificate mode.
...
**OC-R-096** — Which TLS material a `wss://` server uses shall be decided by its `ServerTlsPolicy`: an ephemeral self-signed certificate when `server_cert` resolves to `ServerCertSource::SelfSigned` (the server certificate + key files are then structurally unreachable regardless of any value); otherwise the server certificate + key files when both are set; otherwise the ephemeral fallback of OC-R-095. Cadence for the self-signed pair follows OC-R-037.
...
**OC-R-111** — The OCPP setup dialog's client-side `ca_file` input shall be hidden whenever Skip-Verify is On (mirroring the Modbus TCP dialog), and the resolved config shall exclude `ca_file` from the result whenever Skip-Verify is On, regardless of any text already present in that input. The input widget's stored text shall be left unmodified, so toggling Skip-Verify back Off restores the previously entered CA path. The Skip-Verify toggle itself (client role) shall be shown only when the security level is `Tls` or `MutualTls`, hidden under `NoTls`.

**OC-R-112** — A CSMS's `cert_file` or `key_file` set alone (not both), while `self_signed` is not set, shall fail configuration resolution with a TLS configuration error, rather than silently falling through to an inert listener. The setup dialog's `cert_file`/`key_file`/`client_cert_file`/`client_key_file` inputs shall additionally reject an empty value at entry time (the field cannot be left or submitted blank while shown), not only at resolve time, mirroring MB-R-107.

**OC-R-113** — The OCPP setup dialog's client-CA input, shown for the server role whenever mTLS is selected, shall allow zero or more CA file paths to be added, edited, or removed individually, mirroring the Modbus TCP dialog's equivalent list (MB-R-136). Confirming an add-entry attempt shall be rejected — leaving the entry sub-dialog open with an inline error instead of appending anything — unless the typed path is non-empty, exists on disk, is not a directory, and has one of the accepted certificate-file extensions (`pem`/`crt`/`key`, case-insensitive), mirroring MB-R-136's same gate on its shared add-CA sub-dialog. Selecting mTLS with a non-empty list and Skip Verify Off resolves to `ServerTlsPolicy::MutualTls` with `ClientCertVerification::Verify` holding exactly those files; an empty list at that point is a validation error. The dialog shall additionally offer a Skip Verify toggle for the server (CSMS) role, shown whenever mTLS is selected: when On, the Client CA list input shall be hidden and excluded from the resolved config regardless of entries already present, resolving to `ClientCertVerification::SkipVerify` instead — mirroring MB-R-136's hidden-field-exclusion pattern (list contents preserved, restored when the toggle goes back Off).

**OC-R-115** — Under `ClientTlsPolicy::MutualTls` with `ClientCertSource::SelfSigned`, a CS shall present an ephemeral self-signed certificate/key pair as its mTLS identity, generated and cached per the rule in OC-R-037 (generated once per module instance, reused across reconnects/restarts/config edits, regenerated only on a transition into self-signed), never written to disk.
