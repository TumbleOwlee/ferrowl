...
**OC-R-110** — The OCPP setup dialog shall offer a Self-Signed toggle for the server role, shown whenever the TLS selector (OC-R-127) is TLS or mTLS (mirroring the Modbus TCP dialog's Self-Signed toggle). When On, the server certificate/key inputs (`cert_file`/`key_file`) shall be hidden and the resolved identity shall be `CertSource::SelfSigned` regardless of any text already present in them; the dialog's validation shall not require those files while Self-Signed is On. Self-Signed shall have no effect on the client-CA list or on the selector's mTLS position, which continues to control the `Mutual` variant independently. The input widgets' stored text shall be left unmodified, so toggling Self-Signed back Off restores the previously entered cert/key paths and re-requires them for validation.

**OC-R-111** — The OCPP setup dialog's client-side Root Store toggle and server-CA list (OC-R-125) shall both be hidden whenever Skip-Verify is On (mirroring the Modbus TCP dialog), the resolved verification then being `CertVerification::Skip`, which carries neither. The hidden widgets' stored state — toggle position and list entries alike — shall be left unmodified, so toggling Skip-Verify back Off restores exactly what was previously entered. The Skip-Verify toggle itself (client role) shall be shown only when the TLS selector (OC-R-127) is TLS or mTLS, hidden when it is Off — a connection with TLS off, Basic Authentication (OC-R-128) on or not, has no certificate to verify.
...
**OC-R-127** — The OCPP setup dialog shall present TLS through a single three-way selector — Off / TLS / mTLS — shown for both roles and applying to the instance's own role, mirroring the Modbus TCP dialog's selector, and shall not present any "security level" or profile selection: the selector shall map one-to-one onto the role's policy variant, Off resolving to `ServerTlsPolicy::None`/`ClientTlsPolicy::None`, TLS to `Tls`, and mTLS to `Mutual`, with each variant contributing exactly its own payload (OC-R-110, OC-R-111, OC-R-113, OC-R-116, OC-R-125). Off shall never resolve to a `Tls` variant with a placeholder identity or verification, so a dialog-created station with TLS off carries the same `None` policy a file-created one does and the OC-R-095 fallback applies identically. The Protocol field shall cease to be an input and become a read-only display derived from the selector alone, showing `wss://` whenever the selector is other than Off and `ws://` when it is Off, and the endpoint the dialog resolves shall carry the scheme it displays, so the dialog can never produce the scheme/policy mismatch OC-R-042 and OC-R-097 render inert; the dialog shall carry no explanatory row beneath that display about certificate generation under a `wss` scheme, the mismatch it warned of being unreachable once the scheme follows the selector, self-signed identity being stated where it is chosen (OC-R-110). Changing the selector shall leave every hidden widget's stored state — input text, toggle positions, list entries — unmodified, so returning to a previous selection restores exactly what was entered (mirroring MB-R-135).

**OC-R-128** — The OCPP setup dialog shall offer Basic Authentication as an On/Off selection independent of the TLS selector, shown for both roles, with the username and password inputs presented on the same line as the selection and shown only while it is On. On with both inputs non-empty shall resolve to `username`/`password` set in the `security` table; Off shall resolve to both unset regardless of text already present, the inputs' stored text being left unmodified so switching back to On restores it. Basic Authentication On together with the TLS selector Off shall be a valid combination (Profile 1, OC-R-029), as shall On together with TLS or mTLS, the two axes constraining each other in no way.
