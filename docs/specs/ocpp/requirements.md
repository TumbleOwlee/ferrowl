...
**OC-R-005** — Every CSMS-originated action carries a connector scope `None` (no connector/EVSE field), `Optional` (field exists, may be omitted), or `Required` (mandatory), derived from the presence and optionality of the request's top-level connector/EVSE target.
...
**OC-R-007** — 2.1 is a strict superset of 2.0.1: every 2.0.1 action name exists in 2.1; the 26 additional 2.1 actions are additive.
...
**OC-R-012** — `errorCode` is one of exactly ten spec-fixed codes ([`api-contract.md`](./api-contract.md)). An unrecognized code on the wire is accepted and mapped to `GenericError`, not a frame failure.
...
**OC-R-024** — A malformed inbound frame is logged and does not tear down the connection. Otherwise skipped, except where OC-R-098 requires a CallError.

**OC-R-121** — Teardown does not block on an in-flight inbound Call handler: `shutdown()` aborts every running handler rather than awaiting it. An aborted handler never sends its reply.
...
**OC-R-098** — A frame that fails to decode but is identifiable as a Call (`messageTypeId` 2) with a string `uniqueId` is answered with CallError `FormationViolation` carrying that id. A peer sending a recoverable Call is never left to wait out its own timeout.
...
**OC-R-100** — A malformed CallResult or CallError is never answered, id recoverable or not: a CallError about a CallError is not a valid exchange, and a CallResult has no pending call on the peer to fail.
...
**OC-R-034** — A CS's trust anchors are exactly those its `verification` names: `CertVerification::RootStore` = webpki root store plus every `extra_ca_files` entry; `CertVerification::CaFiles` = exactly the named `ca_files`, not the root store; `CertVerification::Skip` = no anchor (OC-R-036).

**OC-R-035** — A CS presents a client certificate when, and only when, its policy is `ClientTlsPolicy::Mutual`; `Tls` and `None` present none. `CertSource::Files` presents the PEM pair at `cert_file`/`key_file`; `CertSource::SelfSigned` presents OC-R-115's cached ephemeral pair. `CertSource::Ephemeral` rejected at construction as a client identity, mirroring MB-R-110.

**OC-R-036** — A CS with `CertVerification::Skip` accepts any server certificate without authenticating it; the variant names no anchor. Handshake signature verification still performed; only chain/identity check skipped. `RootStore`: verified against the native root store plus every `extra_ca_files` entry. `CaFiles`: against exactly the named `ca_files`, not the native store, `ca_files` non-empty.

**OC-R-037** — A CSMS TLS server certificate comes from the PEM files of `CertSource::Files` or an ephemeral self-signed certificate (`SelfSigned` or `Ephemeral`). The self-signed pair is generated once and cached for the module instance's life, reused across every bind, reconnect-driven rebind, `:restart`/`:reload`, and config edit leaving the identity self-signed — regenerated only when the identity changes to `SelfSigned`/`Ephemeral` from `Files`, or the TLS mode transitions from `None` to `Tls`/`Mutual` while the identity is already self-signed. A torn-down and freshly constructed instance discards the cache. Never written to disk.
...
**OC-R-039** — With a CSMS's `ServerTlsPolicy::Mutual`, client-certificate verification is governed by `verification`. `CaFiles`: a client presenting no certificate, or one not signed by any configured `ca_files`, is rejected — any one CA suffices; `ca_files` non-empty, construction failing otherwise. `RootStore` rejected at construction on this path as client-only. `Skip`: no certificate still rejected, but a presented one accepted without CA validation — signature check performed, chain/identity skipped.

**OC-R-040** — `ServerTlsPolicy::Mutual` with a self-signed CSMS certificate (`identity: CertSource::SelfSigned`) is permitted regardless of `verification` mode — `CaFiles` (≥1 file, construction-checked) or `Skip` — since self-signed identity and client-verification mode are independent. `CaFiles` with zero files remains rejected at construction (OC-R-039), regardless of certificate mode.
...
**OC-R-042** — The endpoint scheme is authoritative for a server's transport: `wss://` binds a TLS-terminated listener; `ws://` binds plain TCP even when a `ServerTlsPolicy` other than `None` is configured — the whole policy, identity and verification, inert. A URL never advertises a transport its listener does not speak.

**OC-R-097** — The scheme is authoritative for a client's transport the same way: a `ws://` CS endpoint connects in plaintext and ignores any TLS material. The roles do not differ in scheme treatment.

**OC-R-095** — A `wss://` **server** endpoint whose `identity` is `CertSource::Ephemeral` ("no TLS material configured") binds with an ephemeral self-signed certificate and reports the fallback in the module log. `CertSource::SelfSigned` binds the same way without logging a fallback (explicit selection). A `wss://` server never silently binds plain TCP.
...
**OC-R-110** — The OCPP setup dialog offers a server-role Self-Signed toggle, shown whenever the TLS selector (OC-R-127) is TLS or mTLS (mirroring the Modbus TCP dialog). On: server `cert_file`/`key_file` inputs hidden, resolved identity `CertSource::SelfSigned` regardless of text; validation does not require those files. No effect on the client-CA list or the selector's mTLS position, which controls `Mutual` independently. Stored text left unmodified, so Off restores the paths and re-requires them.

**OC-R-111** — The dialog's client-side Root Store toggle and server-CA list (OC-R-125) are both hidden whenever Skip-Verify is On (mirroring the Modbus TCP dialog), resolved verification then `CertVerification::Skip`, which carries neither. Hidden widgets' stored state — toggle position, list entries — left unmodified, so Off restores exactly what was entered. The client-role Skip-Verify toggle itself is shown only when the TLS selector (OC-R-127) is TLS or mTLS, hidden at Off — a connection with TLS off, Basic Authentication (OC-R-128) on or not, has no certificate to verify.

**OC-R-112** — A half-configured certificate pair is not representable in a config file: `CertSource::Files` carries `cert_file` and `key_file` as required fields, so naming one without the other fails to deserialize. The rule survives only in the dialog, whose inputs are independently editable: error border on a blank cert/key input while shown; refuses to close on submit while either of a role's pair is blank, CSMS identity and CS mTLS identity alike, mirroring MB-R-107.

**OC-R-113** — The dialog presents CA file paths through one shared list widget — server (CSMS) role: client-CA list (shown whenever mTLS is selected); client (CS) role: server-CA list (OC-R-125) — mirroring MB-R-136, zero or more paths added, edited, or removed individually. Confirming an add-entry attempt is rejected — sub-dialog stays open with an inline error, nothing appended — unless the path is non-empty, exists on disk, is not a directory, and has extension `pem`/`crt`/`key` (case-insensitive), mirroring MB-R-136's gate. Server role: mTLS with a non-empty list and Skip Verify Off → `ServerTlsPolicy::Mutual` with `CertVerification::CaFiles` holding exactly those files; an empty list there is a validation error. The dialog also offers a server (CSMS) role Skip Verify toggle, shown whenever mTLS is selected: On hides the client-CA list and resolves verification to `CertVerification::Skip` regardless of entries — MB-R-136's hidden-field-exclusion pattern (list preserved, restored when Off).

**OC-R-115** — Under `ClientTlsPolicy::Mutual` with `identity: CertSource::SelfSigned`, a CS presents an ephemeral self-signed certificate/key pair as its mTLS identity, generated and cached per OC-R-037 (once per module instance, reused across reconnects/restarts/config edits, regenerated only on a transition into self-signed), never written to disk.

**OC-R-116** — The dialog offers a CS (client) role Self Signed toggle, shown whenever mTLS is selected. On: Client Cert/Key inputs hidden and excluded from the resolved config regardless of text, resolving to `ClientTlsPolicy::Mutual`'s `identity: CertSource::SelfSigned`; validation does not require those files. Stored text left unmodified, so Off restores and re-requires the paths.

**OC-R-125** — The dialog's client (CS) role offers, whenever Skip-Verify is Off and the TLS selector (OC-R-127) is TLS or mTLS, a Root Store toggle (default On) plus the shared CA list widget (OC-R-113) holding zero or more server-CA paths — replacing the single `ca_file` input. Root Store On → `CertVerification::RootStore` with the list as `extra_ca_files`, empty or not; Off → `CertVerification::CaFiles` with the list as `ca_files`, an empty list there being a validation error refusing to close the dialog, mirroring OC-R-113's server-side rule. Skip-Verify On hides both toggle and list per OC-R-111.

**OC-R-126** — The OCPP device security config carries its TLS material as a `tls` sub-block holding one `ServerTlsPolicy` under `server` and one `ClientTlsPolicy` under `client`, serialized `[security.tls.server]`/`[security.tls.client]`, replacing the flat role-mixed certificate fields beside `username`/`password`. `username` and `password` remain flat members of `security`, shared by both roles. Each policy independently defaults to `None`, so an absent `[security.tls]` block is exactly both policies `None`. A CS or CSMS decides whether TLS is configured by matching its own role's policy variant, never by comparing the security block against an all-unset baseline; the other role's policy is inert, never validated against this role's rules. The endpoint scheme remains authoritative over the policy (OC-R-042).

**OC-R-127** — The dialog presents TLS through a single three-way selector — Off / TLS / mTLS — shown for both roles, applying to the instance's own role, mirroring the Modbus TCP dialog, with no "security level" or profile selection: the selector maps one-to-one onto the role's policy variant — Off → `ServerTlsPolicy::None`/`ClientTlsPolicy::None`, TLS → `Tls`, mTLS → `Mutual` — each variant contributing exactly its payload (OC-R-110, OC-R-111, OC-R-113, OC-R-116, OC-R-125). Off never resolves to a `Tls` variant with placeholder identity or verification, so a dialog-created station with TLS off carries the same `None` policy a file-created one does and OC-R-095's fallback applies identically. The Protocol field ceases to be an input and becomes a read-only display derived from the selector alone — `wss://` whenever the selector is other than Off, `ws://` at Off — and the resolved endpoint carries the scheme displayed, so the dialog can never produce the scheme/policy mismatch OC-R-042 and OC-R-097 render inert; the dialog carries no explanatory row beneath that display about certificate generation under `wss`, the mismatch it warned of being unreachable once the scheme follows the selector, self-signed identity being stated where chosen (OC-R-110). Changing the selector leaves every hidden widget's stored state — input text, toggle positions, list entries — unmodified, so returning to a previous selection restores exactly what was entered (mirroring MB-R-135).

**OC-R-128** — The dialog offers Basic Authentication as an On/Off selection independent of the TLS selector, shown for both roles, with username and password inputs on the same line and shown only while On. On with both inputs non-empty → `username`/`password` set in `security`; Off → both unset regardless of text, stored text left unmodified so switching back On restores it. Basic Authentication On with TLS Off is valid (Profile 1, OC-R-029), as is On with TLS or mTLS; the two axes constrain each other in no way.

**OC-R-117** — A CS device config may declare `extra_headers`, an ordered list of `HeaderDef { name, value }`. Every configured header is sent on the WebSocket upgrade in addition to any header the client sets itself (e.g. OC-R-030's `Authorization`, OC-R-004's subprotocol token). Construction rejects a `HeaderDef` whose `name` case-insensitively matches a client-controlled header (`Authorization`, `Host`, `Upgrade`, `Connection`, `Sec-WebSocket-Key`, `Sec-WebSocket-Version`, `Sec-WebSocket-Protocol`, `Sec-WebSocket-Extensions`), naming it in the error.
...
**OC-R-048** — With `reconnect` enabled (default), a CS never ends its task on a failed dial or dropped connection; it waits a backoff and retries, same policy as the Modbus client (MB-R-051). Disabled: a failed dial or dropped connection ends the task with that error, after emitting a disconnected status.
...
**OC-R-107** — `reconnect`, the endpoint, and the security (TLS/auth) configuration are re-read from the shared device config on every dial attempt, so an edit takes effect on the next reconnect without restart (mirrors MB-R-056).

**OC-R-114** — With `reconnect` enabled, a CS logs the failure reason for each failed dial or dropped connection, and logs the wait duration before each backoff wait, mirroring the Modbus client (MB-R-051).

**OC-R-123** — A CS module's displayed status follows MB-R-137's three-state rule: `CONNECTED` while the WebSocket is open; `RECONNECTING` while the task runs but is not connected (dial in progress or backoff wait, OC-R-048); `DISCONNECTED` while the task is not running.
...
**OC-R-051** — Each accepted connection gets an opaque, monotonically increasing connection id from 1. The charge-point identity from the URL path is kept as metadata against that id, **not** used as the connection key — reconnects and duplicate identities never collide.
...
**OC-R-056** — A CSMS answers CS-originated Calls through a handler told which connection the Call arrived on, so one handler serves many stations.
...
**OC-R-064** — An inbound Call the CS simulator does not model is default-accepted with the action's `Default`-derived response, not rejected.
...
**OC-R-070** — A remotely started transaction mints a local transaction id, puts the targeted connector — absent an explicit target, the first — into a charging state, and transmits the transaction-start message (`StartTransaction` for 1.6, `TransactionEvent` with `eventType=Started` for 2.0.1/2.1) through the same send path the RFID/operator flow uses, so the CSMS learns the id via the normal wire message in every case — matching 1.6's pattern of letting the response assign the id where applicable. A remote stop clears the transaction, clears the transaction-scoped charging limit, returns the connector to available.
...
**OC-R-073** — A CSMS module answers every CS-originated Call. Four are crafted: boot notification (accepted, current time, heartbeat interval), heartbeat (current time), authorization (accept/reject status), transaction start (freshly minted unique transaction id plus accept/reject status). Every other → the action's `Default`-derived response.
...
**OC-R-076** — An authorization request (names no connector) is checked against the charge-point-wide list ∪ **every** connector list. A transaction start (names a connector) is checked against that connector's effective set only — one connector's tags do not authorize another's.
...
**OC-R-103** — A charging-station (client) device config persists the CS boot identity (model, vendor, firmware version, serial number) as it persists connectors and configuration keys: `:wd`/`:write-device` writes current values; loading seeds them into CS state, overriding built-in defaults only when the field is present. Absent field → built-in default, so a file predating it still loads.

**OC-R-104** — In OCPP 1.6, charge-point-wide state also carries four optional identity fields: SIM ICCID, SIM IMSI, meter serial number, meter type. Each persisted by `:wd`/`:write-device` and seeded on load as OC-R-103, overriding the built-in default (empty/unset) only when present. Empty → omitted from `BootNotification` entirely; valued → included under its wire name (`iccid`, `imsi`, `meterSerialNumber`, `meterType`). Not applicable to 2.0.1/2.1, which have no equivalent fields.

**OC-R-082** — Connection or listener configuration is rebuilt from the current module spec on every start, so an edited endpoint or security section takes effect on the next start without a stale copy.

**OC-R-083** — A client module does **not** connect automatically; only on explicit start. A server module binds its listener automatically on creation. With `reconnect` enabled (default), a failed bind does not end the module task; it retries with the Modbus client's policy (MB-R-051). Disabled: a failed bind ends the task with that error, surfaced to the caller.

**OC-R-124** — A CSMS server module's displayed status follows MB-R-153's three-state rule: `CONNECTED` while the listener is bound; `RECONNECTING` while the task runs but is not bound (bind in progress or backoff wait, OC-R-083); `DISCONNECTED` while the task is not running.
...
**OC-R-089** — Every action reachable through a send dialog is classified as exactly one of *typed* (flat property table with per-property kind, prefill source, optionality) or *raw JSON*. Neither silent omission nor dual classification.

**OC-R-090** — An action is raw-JSON when, and only when, its request's required fields include a nested object, or a repeated list with no optional escape hatch — shapes the flat table cannot express.
