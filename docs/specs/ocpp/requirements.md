...
**OC-R-113** — The OCPP setup dialog's client-CA input, shown for the server role whenever mTLS is selected, shall allow zero or more CA file paths to be added, edited, or removed individually, mirroring the Modbus TCP dialog's equivalent list (MB-R-136). Selecting mTLS with a non-empty list and Skip Verify Off resolves to `ServerTlsPolicy::MutualTls` with `ClientCertVerification::Verify` holding exactly those files; an empty list at that point is a validation error. The dialog shall additionally offer a Skip Verify toggle for the server (CSMS) role, shown whenever mTLS is selected: when On, the Client CA list input shall be hidden and excluded from the resolved config regardless of entries already present, resolving to `ClientCertVerification::SkipVerify` instead — mirroring MB-R-136's hidden-field-exclusion pattern (list contents preserved, restored when the toggle goes back Off).
