...
| The OCPP setup dialog's client-role Skip-Verify toggle at security level `None`/`BasicAuth` | hidden entirely, not merely inert — Basic-Auth-only connections have nothing to do with certificate verification, so the toggle is shown only at `Tls`/`Mutual` (OC-R-111) |
...
| The OCPP setup dialog's Self-Signed toggle (server role, shown at `Tls`/`Mutual`) toggled On after `cert_file`/`key_file` text was entered | resolved identity becomes `CertSource::SelfSigned`, excluding both files; the widgets' stored text is preserved for when the toggle goes back Off (OC-R-110) |
