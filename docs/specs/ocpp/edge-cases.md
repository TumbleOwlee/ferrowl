...
| A hand-written `ws://` instance whose own-role TLS policy is not `None`, reopened in the OCPP setup dialog | the selector shows TLS or mTLS and the Protocol display shows the derived `wss://`; confirming writes `wss://` back, promoting the inert pairing into a live one. The dialog derives the scheme from the policy (OC-R-127), so it normalises the mismatch rather than preserving it; the instance stays inert (OC-R-042/OC-R-097) only for as long as it is not edited |
...
| A `ws://` CS endpoint whose `ClientTlsPolicy` would fail validation (empty `ca_files`, `Ephemeral` identity, unreadable PEM) | the connection succeeds in plaintext; the policy is neither validated nor loaded, because the scheme decides the transport and the material is inert (OC-R-097), symmetric with the CSMS's scheme gate |
