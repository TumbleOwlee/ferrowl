...
**NF-R-054** — The filesystem-touching call sites that apply `~` expansion (NF-R-042) are: config/session/device config files (`ferrowl::convert::Converter`), CLI `--session`/`--device`/`--module`/`--log-file`, per-module log files, Modbus/OCPP TLS cert/key/CA files (including the setup dialogs' path-existence validation).
