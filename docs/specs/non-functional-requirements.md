...
**NF-R-044** — No test shall read or write a path directly under `std::env::temp_dir()`. A test needing filesystem scratch space shall obtain it from `reserve_temp_dir(prefix: &str) -> TempDirGuard`, which creates a directory under `std::env::temp_dir()` whose name is unique per run — `prefix`, the current process id, and a monotonically increasing per-process counter — and exposes `path() -> &std::path::Path` and `join(name: impl AsRef<Path>) -> std::path::PathBuf`. Creation is exclusive: the guard shall fail if the derived directory already exists and retry with the next counter value, so a stale directory left by an earlier process whose id has since been reused is never adopted, and shall panic rather than hand back a directory it did not create. Dropping the guard removes the directory and its contents best-effort: a removal failure is ignored, never a panic, so a cleanup race cannot fail an otherwise passing test. Fixed filenames remain legible because they are joined onto the guard's unique directory rather than onto the shared temp root, so two concurrent test binaries, two checkouts of the workspace, and a rerun overlapping the previous run's cleanup never address the same absolute path. Exempt are `ferrowl-test-support`'s own tests of `reserve_temp_dir`/`TempDirGuard`, which address paths directly under `std::env::temp_dir()` — pre-creating a derived directory to force the exclusive-creation retry, removing a guard's directory behind its back to prove drop tolerates a missing one — there the raw temp path is the assertion.
