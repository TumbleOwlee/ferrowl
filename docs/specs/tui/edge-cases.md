...
| **UI-E-162** | `:stop` issued during a pending configuration edit apply (UI-R-351, UI-R-353) | the module reports stopped immediately while its configuration is still changing underneath: the view keeps rendering the pre-edit configuration until the stop settles (UI-E-161), then shows the edited configuration on a stopped module. A module that is stopped and simultaneously adopting a new configuration is the price of honouring the confirmed edit; discarding the edit instead is the silent data loss this rule removes |
| **UI-E-163** | `:reload` issued during a pending configuration edit apply (UI-R-354) | the confirmed edit is discarded even though its dialog already reported the apply accepted, the Warning line in the module's message log being the user's only signal. Deliberate: `:reload` names the on-disk configuration as the wanted one, and silently keeping the edit would make `:reload` not reload |
