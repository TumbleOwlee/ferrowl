...
**UI-R-016** — The command line is parsed by a pure, state-independent parser into a fixed set of app-level commands ([`api-contract.md`](./api-contract.md) §1); leading/trailing and inter-token whitespace collapsed on split. Any first token not recognized at the app level is classified unknown and forwarded verbatim to the active view.
