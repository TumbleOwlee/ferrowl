...
/// Drive the retry loop: bind the configured TCP address and serve using ASCII framing
/// (MB-R-126), retrying a bind or mid-serve failure per `BackoffPolicy` when `config.reconnect`
/// is set (MB-R-126, MB-R-130–134). Each accepted connection answers from the shared `memory`
/// (verbose logging on, MB-R-067). Plain TCP unless `config.tls` is set (MB-R-127), in which
/// case the listener terminates TLS on each accepted connection. Thin wrapper over
/// [`run_tcp_family`](crate::server_core::run_tcp_family), shared with `tcp::server` and
/// `rtu_over_tcp::server` — the three transports differ only in which framing they pass.
async fn run<T, L, St>(
    config: Arc<RwLock<Config>>,
    memory: Arc<MemLock<Memory<Key<T>>>>,
    receiver: Receiver<ServerCommand>,
...
where
    ...
    St: LogFn + Clone,
{
    run_tcp_family::<T, rust_modbus::Ascii, L, St>(
        config,
        memory,
        receiver,
        log,
...
mod tests {
    ...
    /// before the first successful bind, `Some(<real addr>)` once bound, `None` again once
    /// `ServerCommand::Terminate` ends the serve loop.
    #[tokio::test]
