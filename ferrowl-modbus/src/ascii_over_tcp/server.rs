...
/// (MB-R-126), retrying a bind or mid-serve failure per [`BackoffPolicy`] when
/// `config.reconnect` is set (MB-R-126, MB-R-130–134). Each accepted connection answers from
/// the shared `memory` via a [`Server`] (verbose logging on, MB-R-067). Plain TCP unless
/// `config.tls` is set (MB-R-127), in which case the listener terminates TLS on each accepted
/// connection.
async fn run<T, L, St>(
    config: Arc<RwLock<Config>>,
    memory: Arc<MemLock<Memory<Key<T>>>>,
                    ...
                    match TcpListener::bind(addr).await {
                        ...
                        Ok(listener) => {
                            ...
                            *bound_addr.lock() = Some(bound);
                            ...
                            let end = drive_serve(
                                server.serve_framed::<rust_modbus::Ascii>(listener),
                                handle,
                                &mut receiver,
                            )
                            .await;
                            ...
                            match TlsListener::bind(addr, tls_config).await {
                                ...
                                Ok(listener) => {
                                    ...
                                    *bound_addr.lock() = Some(bound);
                                    ...
                                    let mut receiver = receiver.lock().await;
                                    let end = drive_serve(
                                        server.serve_tls::<rust_modbus::Ascii>(listener),
                                        handle,
                                        &mut receiver,
                                    )
                                    .await;
