...
/// Open the configured serial port and serve it under Ascii framing (MB-R-124 revised). Thin
/// wrapper over [`run_serial_family`](crate::server_core::run_serial_family), shared with
/// `rtu::server` — the two transports differ only in which framing they pass.
async fn run<T, L, St>(
    config: Arc<RwLock<Config>>,
    memory: Arc<MemLock<Memory<Key<T>>>>,
    receiver: Receiver<ServerCommand>,
...
where
    ...
    St: LogFn + Clone,
{
    run_serial_family::<T, rust_modbus::Ascii, L, St>(
        config,
        memory,
        receiver,
        log,
