...
/// MB-R-141 — open the configured serial port receive-only under Ascii framing and drive the
/// monitor's decode/match loop. Thin wrapper over
/// [`run_serial_monitor`](crate::monitor::run_serial_monitor), shared with `rtu::monitor` — the
/// two transports differ only in which framing they pass.
