...
/// (MB-R-113), retrying a bind or mid-serve failure per `BackoffPolicy` when `config.reconnect`
/// is set (MB-R-114, MB-R-130–134). Each accepted connection answers from the shared `memory`
/// (verbose logging on, MB-R-067). Plain TCP unless `config.tls` is set (MB-R-115), in which
...
/// `ascii_over_tcp::server` — the three transports differ only in which framing they pass.
async fn run<T, L, St>(
    config: Arc<RwLock<Config>>,
    memory: Arc<MemLock<Memory<Key<T>>>>,
    receiver: Receiver<ServerCommand>,
...
where
    ...
    St: LogFn + Clone,
{
    run_tcp_family::<T, rust_modbus::RtuOverTcp, L, St>(
        config,
        memory,
        receiver,
        log,
