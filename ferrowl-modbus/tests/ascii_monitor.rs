//! ASCII monitor open-failure tests, mirroring `rtu_monitor.rs`'s scope note: a real serial
//! loopback needs hardware, which isn't portable in CI, so these cover the serial-open failure
//! path — where the retry/backoff lifecycle (MB-R-141, MB-R-130–134) is observable.
...
/// is built over `ferrowl_modbus::rtu::Config` (same fields, differing only in on-wire framing —
/// see `ascii/monitor.rs`'s own doc comment).
fn bad_config(reconnect: bool) -> ferrowl_modbus::rtu::Config {
