...
async fn ut_monitor_open_failure_retries_while_reconnect_enabled() {
    ...
    let handle = rtu::MonitorBuilder::new(Arc::new(RwLock::new(bad_config(true))), empty_table())
        .spawn(rx, sink(), sink())
        .await
...
async fn ut_monitor_open_failure_reconnect_false_ends_task() {
    let (_tx, rx) = mpsc::channel::<ServerCommand>(1);
    let handle = rtu::MonitorBuilder::new(Arc::new(RwLock::new(bad_config(false))), empty_table())
        .spawn(rx, sink(), sink())
        .await
        .expect("spawn always returns Ok");
