...
`issue.md`: line 1 title (plain language, no slug/ID), rest body. Self-contained — every new requirement's full text beside its ID, every changed one old → new, plus the other spec entries. `## Background`/`## Why`, `## Scope`, `## Goal`. Goal and normative changes only — never file/function/approach. Compact ID ranges.
