...
- **Modbus** — client, server, and a passive bus monitor, over TCP, UDP, RTU and
  ASCII (serial), and RTU/ASCII framing tunnelled over TCP; TCP-based transports
  optionally under TLS or mutual TLS.
...
A separate headless bridge mode (`ferrowl bridge`) relays Modbus traffic between
two independently configured interfaces, e.g. a TCP-only master in front of a
serial-only device, without the TUI, session file, or scripting.
...
- Run the same simulation logic interactively (TUI) or headlessly (`ferrowl run`,
  for CI).
- Observe and relay real Modbus traffic, not only simulate it: watch a serial bus
  without participating, or stand between a master and a device as a transparent
  bridge.
...
6. **Modbus bus observation** — attach a read-only monitor to an RTU or ASCII
   serial bus and watch the request/response pairs other devices exchange, without
   ever transmitting.
7. **Modbus bridging** — `ferrowl bridge --upstream … --downstream …` sits between
   a master and a device on different transports (TCP, RTU, RTU/ASCII over TCP),
   forwarding every request and answer unmodified, optionally restricted to a set
   of unit ids on a shared bus.
8. **Config migration** — `ferrowl migrate` brings a pre-v0.4.0 (`modbus-cli-rs`)
...
|---|---|---|
| Modbus | Client, server & passive monitor roles; TCP, UDP, RTU, ASCII, RTU-over-TCP, ASCII-over-TCP; TLS + mutual TLS on TCP-based transports; all four register tables; typed registers (13 formats, both endians, bit-fields, display scaling); client/server auto-reconnect with backoff. | [`docs/specs/modbus/`](./docs/specs/modbus/) |
...
| Bridge | `ferrowl bridge`: headless Modbus relay between one upstream (server-side) and one downstream (client-side) interface, any TCP/RTU transport mix, optional TLS and unit-id filtering. | [`docs/specs/bridge/`](./docs/specs/bridge/) |
...
| RTU / ASCII | The two Modbus serial framings; either can also be tunnelled over a TCP socket (RTU-over-TCP, ASCII-over-TCP). |
| Monitor | The third Modbus role: a receive-only observer of an RTU/ASCII serial bus that decodes other devices' traffic and never transmits. |
| Bridge | `ferrowl bridge` — a headless relay that serves Modbus on its upstream interface and forwards every request to its downstream interface as a client. |
| Upstream / Downstream | The bridge's two interfaces: upstream faces the master (bridge acts as server), downstream faces the device (bridge acts as client). |
...
| Headless mode | `ferrowl run` — runs modules and Lua sims without a terminal UI, for CI. `ferrowl bridge` is headless too but runs no modules or sims. |
