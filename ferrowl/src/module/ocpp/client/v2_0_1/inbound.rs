//! OCPP 2.0.1 inbound (CSMS→CS) decision logic — the fully-typed [`Inbound`] impl the generic
//! [`CsStateHandler`](crate::module::ocpp::client::handler) delegates to. It matches the typed
//! `Action201` enum, reads typed request fields, mutates the shared [`CsState`], and builds typed
//! `rust_ocpp::v2_0_1` responses; no `serde_json::Value` is touched. GetVariables is answered from
//! the variable store, SetVariables writes it, Reset mutates state, EVSE-scoped Calls are simulated
//! against the connector on the targeted EVSE, and every other inbound Call is default-accepted
//! (see `UNHANDLED.md`). The 2.1 twin (`v2_1/inbound.rs`) is a deliberate near-copy — identical
//! decision logic over the 2.1 types.
...
use ferrowl_ocpp::v2_0_1::enumerations::charging_profile_purpose::ChargingProfilePurposeEnumType;
use ferrowl_ocpp::v2_0_1::enumerations::charging_profile_status::ChargingProfileStatusEnumType;
use ferrowl_ocpp::v2_0_1::enumerations::charging_rate_unit::ChargingRateUnitEnumType;
use ferrowl_ocpp::v2_0_1::enumerations::get_variable_status::GetVariableStatusEnumType;
use ferrowl_ocpp::v2_0_1::enumerations::operational_status::OperationalStatusEnumType;
...
impl Inbound for V2_0_1 {
    fn respond(
        ...
        match action {
            ...
            Action201::SetVariables(req) => with_state_mut(state, |s| {
            ...
            Action201::Reset(_) => with_state_mut(state, |s| {
            ...
            Action201::SetChargingProfile(req) => {
                ...
                with_state_mut(state, |s| {
                        ...
                        && stack > max
                    {
                        ...
                        let period = profile
                            .charging_schedule
                            .first()
                            .and_then(|sc| sc.charging_schedule_period.first().map(|p| (sc, p)));
                        let context = if let Some((schedule, period)) = period {
                            let limit: f64 = period.limit.to_string().parse().unwrap_or(0.0);
            ...
            Action201::CancelReservation(req) => with_state_mut(state, |s| {
            ...
            Action201::ChangeAvailability(req) => {
            ...
            Action201::RequestStartTransaction(req) => with_state_mut(state, |s| {
            ...
            Action201::RequestStopTransaction(req) => {
            ...
            Action201::ClearChargingProfile(req) => {
            ...
            Action201::UnlockConnector(req) => with_state_mut(state, |s| {
