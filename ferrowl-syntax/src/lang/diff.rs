...
mod tests {
    ...
    #[test]
    /// UI-R-037 — Diff is a third full language under `highlight_line`'s contract: sorted,
    /// non-overlapping, char-indexed spans over the crate's public entry point.
    fn ut_diff_is_a_third_highlightable_language() {
        let (spans, _) = crate::highlight_line(Language::Diff, "+added", LineState::default());
        assert_eq!(spans, vec![(0, 6, SyntaxKind::Added)]);
        for w in spans.windows(2) {
            assert!(w[0].1 <= w[1].0, "spans must be sorted and non-overlapping");
        }
    }

    #[test]
    /// UI-R-114 — a Diff line yields at most one span covering the whole line.
    ...
    #[test]
    /// UI-R-115 — a `+`-prefixed line is classified diff added.
    ...
    #[test]
    /// UI-R-116 — a `-`-prefixed line is classified diff removed.
    ...
    #[test]
    /// UI-R-117 — each of the five meta prefixes classifies as diff meta.
    ...
    #[test]
    /// UI-R-118 — a context line, an empty line, or arbitrary text yields no span at all.
    ...
    #[test]
    /// UI-R-119 — Diff highlighting neither reads nor changes the carry-over line state.
    ...
    #[test]
    /// UI-E-065 — `---`/`+++` classify as meta, not removed/added, because meta is matched first.
    ...
    #[test]
    /// UI-E-066 — a lone `+` or `-` yields a one-character span of the added/removed kind.
